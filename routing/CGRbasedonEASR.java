...
public class CGRbasedonEASR extends ActiveRouter{
	/**自己定义的变量和映射等
	 * 
	 */
	 ...
	 * @param s
	 */
	public CGRbasedonEASR (Settings s){
		super(s);
	}
	/**
	 * 初始化
	 * @param r
	 */
	protected CGRbasedonEASR(CGRbasedonEASR  r) {
		super(r);
	}
	/**
	 * 复制此router类
...
//				
				...
				int nextTime = arrivalTime.get(host).intValue();
				
				for (int endTime = nextTime + msgTtl; nextTime <= endTime; nextTime++){
					...
					startTime.put(oneNeighborHost, linkSetUpTime); 										
				}
