...
public abstract class MessageRouter {
	...
	public Message messageTransferred(String id, DTNHost from) {
		...
		//if(getApplications(incoming.getAppID()).size() == 0)
		//	System.out.println("app size 0  "+incoming.getAppID());
		
		Message aMessage = (outgoing==null)?(incoming):(outgoing);//如果outgoing为空，即app处理到来的包后没有要传出去的包
		// If the application re-targets the message (changes 'to')
		// then the message is not considered as 'delivered' to this host.
		isFinalRecipient = aMessage.getTo() == this.host;//如果amessage==incoming Message，就为真，即最后接收位isFinalRecipient 为真
		isFirstDelivery = isFinalRecipient &&//isFirstDelivery为真的条件是，没有要回传的outgoing包，且deliveredMessages列表里还没有添加传入的incoming包（即代表incoming包第一次发来）
		!isDeliveredMessage(aMessage);//检查isDeliveredMessage列表里面有没有aMessage，若有返回真。如果是第一次收到incoming包，则应该是不存在的，即代表incoming包第一次发来

		if (!isFinalRecipient && outgoing!=null) {//如果有要传出去的数据包，即outgoing不为空，则把对应的outgoing包加到router的messages列表里面
			// not the final recipient and app doesn't want to drop the message
			// -> put to buffer
			System.out.println("is not FinalRecipient, relay!");
