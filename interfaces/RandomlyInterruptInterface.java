...
public class RandomlyInterruptInterface  extends NetworkInterface {
	...
	public void update() {
		...
		double probabilityOfInterrupt = s.getDouble("probabilityOfInterrupt");//读取用户设置的链路中断概率
		if (probabilisticInterrupt(probabilityOfInterrupt)) {//随机选择某一个时刻断开连接
			...
			for (int i=0; i < this.connections.size(); ) {
				...
				if (probabilisticInterrupt(probabilityOfInterrupt)) {//随机断开连接
